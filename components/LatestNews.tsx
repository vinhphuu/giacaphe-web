...
async function getLatestArticles(): Promise<Article[]> {
  ...
  try {
    ...
    const{data,error}=await sb.from("articles").select("title,slug,meta_description,created_at")
      .eq("status","published").order("created_at",{ascending:false}).limit(3);
    if(error) return [];
...
// Toàn bộ card là 1 Link — KHÔNG có thẻ <a> lồng nhau bên trong
function ArticleCard({ article }: { article:Article }) {
  return (
    <Link
      href={`/tin-tuc/${article.slug}`}
      title={article.title}
      className="group relative flex flex-col rounded-2xl border border-slate-800
                 bg-slate-900/60 p-5 cursor-pointer
                 hover:border-amber-500/40 hover:bg-slate-800/80
                 transition-all duration-300 overflow-hidden"
    >
      {/* Glow */}
      <div className="pointer-events-none absolute -top-10 -left-10 w-32 h-32 rounded-full
                      bg-amber-500/0 group-hover:bg-amber-500/8 blur-2xl transition-all duration-500"/>

      {/* Badge */}
      <div className="flex items-center gap-1.5 mb-3">
        <span className="inline-flex items-center gap-1 text-[10px] font-semibold
                         text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-full
      ...
      {/* Tiêu đề — hover đổi màu vàng + gạch chân nhẹ */}
      <h3 className="font-bold text-white text-[15px] leading-snug mb-2
                     group-hover:text-amber-400 group-hover:underline
                     decoration-amber-400/40 underline-offset-2
      ...
      {/* Footer — chỉ dùng <span> để tránh lồng <a> */}
      <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between gap-4">
        ...
        {/* span thay vì <a> để tránh lồng nhau */}
    ...
    <section className="w-full px-4 pb-10 mt-2">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-5">
          ...
          {!isEmpty && (
            <Link href="/tin-tuc" title="Xem tất cả bài phân tích thị trường cà phê"
        ...
        {isEmpty && (
          <p className="text-center text-slate-600 text-xs mt-4">
            Bài viết sẽ tự động xuất hiện sau khi AI chạy lần đầu.
          </p>
        )}
      </div>
    </section>
