...
export interface PriceData {
  province: string; price: number; change_value: number; region: string;
}
export interface HistoryPoint { date: string; price: number; }
export interface GeneratedArticle {
  title: string; slug: string; content: string;
  meta_description: string; excerpt: string; keywords: string[]; word_count: number;
}
...
function getTodayVN() {
  return new Date().toLocaleDateString("vi-VN", { weekday:"long",day:"2-digit",month:"2-digit",year:"numeric",timeZone:"Asia/Ho_Chi_Minh" });
}
...
Ngôn ngữ chuyên gia tài chính. Từ khoá: "giá cà phê hôm nay","xuất khẩu cà phê","nguồn cung Tây Nguyên","sàn London","tỷ giá USD". Kết thúc bằng CTA theo dõi giacaphe.com.

Trả về JSON (không có markdown bên ngoài):
{"title":"...","meta_description":"150-160 ký tự...","excerpt":"sapo 100-120 chữ...","content":"...toàn bộ markdown...","keywords":["..."]}`;
}
...
async function callGeminiAPI(prompt: string) {
  ...
  if (!text) throw new Error("Gemini không trả về content");
  return text;
}
...
export async function generateMarketArticle(prices: PriceData[], history: HistoryPoint[]): Promise<WriteResult> {
  ...
  let rawJson: string;
  try { rawJson = await callGeminiAPI(buildPrompt(prices, history)); }
  catch(err) { return {success:false,article:null,error:`Gemini lỗi: ${err instanceof Error?err.message:"Unknown"}`}; }
